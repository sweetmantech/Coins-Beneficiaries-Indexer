import {
  ...
  type SoundEditionV2_1Transfers_Minted_handlerArgs,
  type SoundEditionV2_1Transfers_Airdropped_handlerArgs,
  type Transfers,
} from "generated";

// Covers all mints: user purchases (via SuperMinterV2) and direct admin mints
SoundEditionV2_1Transfers.Minted.handler(
  async ({ event, context }: SoundEditionV2_1Transfers_Minted_handlerArgs) => {
...
SoundEditionV2_1Transfers.Airdropped.handler(
  async ({ event, context }: SoundEditionV2_1Transfers_Airdropped_handlerArgs) => {
    const collection = event.srcAddress.toLowerCase();
    const tier = BigInt(event.params.tier);
